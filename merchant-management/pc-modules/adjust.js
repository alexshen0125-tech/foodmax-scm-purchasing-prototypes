/* PC · 店铺运营平台 —— 业务调整单（通用正/负向款项单据）+ 调整类型配置
   ...
   - 两套正交状态，序列化值大小写刻意不同，不得混用：
       业务状态 status   ：effective 已生效 / reversed 已冲正（小写）
...
function pushTag(r){ const d=pushDisplay(r);
...
function statusTag(r){ return r.status=='reversed'
  ? '<span class="tag t-gr"><span class="dot"></span>已冲正</span>'
  : '<span class="tag t-g"><span class="dot"></span>已生效</span>'; }
...
const selectable = r => r.pushStatus=='PENDING';   // 同罚款单 isRowSelectable：只有待推送可勾
...
window.adj_detail=function(no){
  ...
  <div class="drawer-ft">
    ...
    ${canReverse?`<button class="btn btn-d" onclick="adj_reverseAsk('${r.adjustNo}')">红冲此单</button>`:''}
