/* PC · 店铺运营平台 —— 业务调整单（通用正/负向款项单据）+ 调整类型配置
   ...
   - 两套正交状态，序列化值大小写刻意不同，不得混用：
       业务状态 status   ：effective 已生效 / reversed 已冲正 / voided 已作废（小写）
   ...
   - 错单处理按是否已推送二分（2026-09-14 沈亮定）：
       未推送（PENDING）→ 作废：单据置 voided，永不推送，下游无感知；作废原因必填。
       已推送（PUSHED） → 红冲：生成反向单再推一笔，下游自行抵消。
     作废红冲单 → 原单恢复 effective。系统来源单（罚款）不在此作废，走罚款单自身撤销。
...
// 资金方向 = 类型方向 × 是否红冲
function adjDir(r){ const base=tOf(r.adjustTypeCode).dir; return r.reversal ? (base=='ADDITION'?'DEDUCTION':'ADDITION') : base; }
...
function statusTag(r){
  ...
  if(r.status=='reversed') return '<span class="tag t-gr"><span class="dot"></span>已冲正</span>';
...
const canReverse = r => r.sourceType=='MANUAL' && r.pushStatus=='PUSHED'  && r.status=='effective' && !r.reversal;
...
PAGES['p-adjust']=()=>{
  ...
  <div class="card" style="margin-bottom:14px"><div class="card-bd" style="padding:16px 20px 12px">
    ...
    <div class="fg3">
      ...
      <div class="fr"><label class="fl">状态</label><select id="af-st"><option value="">全部</option><option value="effective" ${f.st=='effective'?'selected':''}>已生效</option><option value="reversed" ${f.st=='reversed'?'selected':''}>已冲正</option><option value="voided" ${f.st=='voided'?'selected':''}>已作废</option></select></div>
    </div>
  ...
  <div class="card">
    ...
    <div class="card-bd flush"><div style="overflow-x:auto"><table>
      ...
      ${rows.map(r=>`<tr>
        ...
        <td style="white-space:nowrap">${r.adjustTypeName}${r.reversal?' <span class="tag t-y" style="font-size:10.5px"><span class="dot"></span>红冲</span>':''}</td>
      ...
      <div class="card-bd" style="border-top:1px solid var(--bd2);font-size:12.5px;color:var(--ts)">
        业务调整单只负责<b>产生一笔正/负向款项</b>并推送给下游，<b>不涉及结算归期、结算单与开票</b>。建单即时生效、不可修改；错单处理：<b>未推送的直接作废</b>，<b>已推送的走红冲</b>。
...
window.adj_typeChange=function(){
  ...
  const d=document.getElementById('an-dir'); if(d) d.innerHTML=dirTag({adjustTypeCode:t.code,reversal:0});
...
window.adj_newConfirm=function(){
  ...
  <div class="mc-bd">
    ...
    <div style="border:1px solid var(--bd);border-radius:8px;overflow:hidden;margin:10px 0 12px"><table style="margin:0"><tbody>
      ...
      <tr><td style="color:var(--ts)">方向</td><td>${dirTag({adjustTypeCode:tc,reversal:0})}</td></tr>
...
window.adj_detail=function(no){
  ...
  const reversedBy=(DB.adjOrders||[]).find(x=>x.reversalOfAdjustNo==r.adjustNo);
  ...
  <div class="drawer-bd">
    <div class="row" style="gap:8px;margin-bottom:6px">${dirTag(r)}${srcTag(r)}${r.reversal?'<span class="tag t-y"><span class="dot"></span>红冲单</span>':''}${statusTag(r)}${pushTag(r)}</div>
  ...
  <div class="drawer-ft">
    ...
    ${canReverse(r)?`<button class="btn btn-d" onclick="adj_reverseAsk('${r.adjustNo}')">红冲此单</button>`:''}
...
window.adj_voidAsk=function(no){
  ...
  const s=adjSign(r);
  const orig=r.reversalOfAdjustNo?adjOf(r.reversalOfAdjustNo):null;
  ...
  <div class="mc-bd">
    ...
    <div style="border:1px solid var(--bd);border-radius:8px;overflow:hidden;margin:10px 0 12px"><table style="margin:0"><tbody>
      ...
      <tr><td style="color:var(--ts)">调整类型</td><td>${r.adjustTypeName}${r.reversal?' · 红冲':''}</td></tr>
      <tr><td style="color:var(--ts)">金额</td><td><span style="color:${s>0?'var(--g)':'var(--r)'};font-weight:700">${s>0?'+':'-'}${money(r.amount)}</span></td></tr>
    </tbody></table></div>
    ${orig?`<div class="ib ib-b"><span class="i">ℹ️</span>这是 <span class="mono">${orig.adjustNo}</span> 的红冲单，作废后原单<b>恢复为已生效</b>。</div>`:''}
...
window.adj_voidDo=function(no){
  ...
  if(!(r.pushStatus=='PENDING'&&r.status=='effective')){closeModal();render();toast('该单已被推送或已作废，无法作废；已推送的错单请走红冲','err');return;}
  ...
  addLog(no,'作废，原因：'+why);
  if(r.reversalOfAdjustNo){
    const o=(DB.adjOrders||[]).find(x=>x.adjustNo==r.reversalOfAdjustNo);
    if(o&&o.status=='reversed'){ o.status='effective'; addLog(o.adjustNo,'红冲单 '+no+' 已作废，恢复为已生效'); }
  }
  DB.adjSel=DB.adjSel.filter(x=>x!=no);
  closeModal();closeDrawer();render();
  toast('调整单 '+no+' 已作废'+(r.reversalOfAdjustNo?'，原单恢复为已生效':''),'ok');
};

/* ================= 红冲（仅已推送） ================= */
window.adj_reverseAsk=function(no){
  const r=adjOf(no); if(!r||!canReverse(r)) return;
  const s=adjSign(r);
  modal(`<div class="mc-hd"><h3>红冲调整单</h3><p>原单不修改、不删除；系统生成一张反向单抵消</p><button class="mc-x" onclick="closeModal()">×</button></div>
  <div class="mc-bd">
    <div class="ib ib-y"><span class="i">⚠️</span>红冲<b>不是撤销</b>：原单已推送的款项不会被收回，下游拿到的是<b>一正一负两笔</b>，自行抵消。</div>
    <div style="border:1px solid var(--bd);border-radius:8px;overflow:hidden;margin:10px 0 12px"><table style="margin:0"><thead><tr><th>单据</th><th>类型</th><th style="text-align:right">金额</th></tr></thead><tbody>
      <tr><td class="mono">${r.adjustNo}</td><td>${r.adjustTypeName}（原单）</td><td style="text-align:right;color:${s>0?'var(--g)':'var(--r)'};font-weight:600">${s>0?'+':'-'}${money(r.amount)}</td></tr>
      <tr><td class="mono" style="color:var(--ts)">待生成</td><td>${r.adjustTypeName} · <b>红冲</b></td><td style="text-align:right;color:${s>0?'var(--r)':'var(--g)'};font-weight:600">${s>0?'-':'+'}${money(r.amount)}</td></tr>
  ...
  <div class="mc-ft"><button class="btn btn-o" onclick="closeModal()">取消</button><button class="btn btn-d" onclick="adj_reverseDo('${r.adjustNo}')">确认红冲</button></div>`);
};
window.adj_reverseDo=function(no){
  ...
  closeModal();closeDrawer();render();
  toast('已生成红冲单 '+rn+'，原单 '+r.adjustNo+' 转为已冲正','ok');
};
