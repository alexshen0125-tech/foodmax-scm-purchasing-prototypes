  ...
  function refView(){
    ...
    const body=skus.map(s=>{idx++;const ents=Object.entries(s.whs);const total=ents.reduce((a,[,q])=>a+q,0);const rs=ents.length;
      return ents.map(([wh,q],wi)=>{
        ...
        const psCols=psOn?`<td style="text-align:right">${pn.left?`<span style="color:var(--g)">${pn.left}</span>`:'<span style="color:var(--tt)">—</span>'}</td><td style="text-align:right">${pn.gross?`<span style="color:var(--gold)">+${pn.net}</span>${pn.ded?`<div style="font-size:11px;color:var(--ts)">预测 ${pn.gross} − 在仓 ${pn.ded}</div>`:''}`:'<span style="color:var(--tt)">—</span>'}</td><td style="text-align:right"><b>${should}</b> ${s.unit}${pn.gross&&!pn.net?'<div style="font-size:11px;color:var(--g)">在仓已够·免送预送货</div>':''}</td>`:'';
    ...
    <div class="card-bd flush"><div style="overflow-x:auto"><table>
      <thead><tr><th style="width:44px">序号</th><th>商品名称</th><th>规格</th><th>分类</th><th style="text-align:right">合计销量</th><th>仓库</th><th style="text-align:right">库存总数</th><th style="text-align:right">昨日销量</th><th style="text-align:right">订单量</th>${psOn?'<th style="text-align:right">在仓剩余</th><th style="text-align:right">预送量</th><th style="text-align:right">今日应送</th>':''}</tr></thead>
