/* PC · 备货单（JH）—— 单据链核心枢纽：销售订单 → 备货单 → 送货单。
   备货单由系统按「送达日」自动成单（一天一张）（ensurePickOrders，定义在主文件全局）。
   本模块 = 备货单列表 + 备货单详情（① 按SKU汇总备货 → ② 按订单×SKU 分拣贴码 → 贴码完触发送货单）。
...
(function(){
  ...
  window.pick_done=function(id){const p=DB.pickOrders.find(x=>x.id==id);if(!p)return;if(p.status=='备货中')p.status='已备货';render();toast(`${id} 已按 SKU 备齐，可开始分拣贴码`,'ok');};
  // 贴码完成 → 生成送货单（按仓拆）
  ...
  function listView(){
    ...
    return `<div class="ib ib-b" style="margin-bottom:14px"><span class="i">🧺</span><div><b>备货单</b>：系统按「送达日」把当天订单汇总成一张，跨仓按 SKU 备一次；备完按订单贴码，贴码完成后按<b>入库仓库</b>自动拆成送货单。</div></div>
  ...
  function detailView(p){
    ...
    <div class="card"><div class="card-hd"><h3>② 分拣贴码 · 按订单</h3><span class="sub">备完按订单分货，每单点「打印条码」一次打出该单全部 SKU 条码贴上（每单每SKU一张）→ 转已贴码；全部贴完 → 生成送货单</span></div><div class="card-bd flush"><div style="overflow-x:auto"><table>
      ...
      ${os.map(o=>{const i=DB.orders.indexOf(o);const done=o.status=='packed'||o.deliveryId;return `<tr>
        ...
        <td>${done?`<button class="btn btn-o btn-sm" onclick="ord_printLabels(${i})">重印条码</button>`:`<button class="btn btn-p btn-sm" ${p.status=='待备货'?'disabled title="请先确认备完"':''} onclick="ord_relabel(${i})">🏷️ 打印条码</button>`}</td>
