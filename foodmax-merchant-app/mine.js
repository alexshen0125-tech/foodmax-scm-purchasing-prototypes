...
let STORE_HALAL_REJECT='';      // 最近一次驳回原因（运营驳回回写）
...
function openStore(){
  pushPage({title:'店铺信息',body:`
    ...
    <div id="st-hl-cert">
      <div class="mn-fld"><div class="fk"><span class="req">*</span>HALAL 证书编号</div><input class="fin" id="st-hl-no" maxlength="50" placeholder="请输入证书上的编号，最多 50 字符" value="${STORE_HALAL.certNo||''}"></div>
    ...
    mount:(p)=>{
      ...
      const blockMsg=l=>`<div style="margin-top:10px;padding:10px 12px;border-radius:12px;background:#FCEDEB;color:#A33A31;font-size:12.5px;line-height:1.6"><b>店铺下还有 ${l.length} 个 HALAL 商品，不能改为「否」。</b><br>${l.slice(0,5).map(g=>`${g.n}${g.code?`（${g.code}）`:''}`).join('、')}${l.length>5?` 等 ${l.length} 个`:''}<br>请先在商品管理中将这些商品的「是否HALAL」改为非HALAL，再修改店铺设置。<div class="st-hl-jump" style="margin-top:6px;font-weight:700;color:var(--g,#0E7A52)">去商品管理 →</div></div>`;
      const pick=v=>{errBox.innerHTML='';
        if(v==='否'&&STORE_HALAL.on==='是'){const l=halalGoods();if(l.length){hl.on='是';seg();errBox.innerHTML=blockMsg(l);const j=errBox.querySelector('.st-hl-jump');if(j)j.onclick=()=>{window.FM_GOODS_FILTER={halal:1};window.FM_MOD.goods();};return false;}}
      ...
      if(STORE_HALAL_PENDING){   // 审核中两端只读（BR-03）：禁用 HALAL 区块并隐藏提交
        p.querySelectorAll('.st-hl-opt').forEach(o=>{o.onclick=null;o.style.opacity='.6';});
        p.querySelectorAll('#st-hl-cert input,#st-abbr').forEach(i=>i.disabled=true);
        p.querySelectorAll('#st-hl-files [data-del],#st-hl-add').forEach(e=>e.style.display='none');
        const sb=document.querySelectorAll('#st-sub');const b=sb[sb.length-1];if(b)b.style.display='none';
      }
      ...
      p.querySelector('#st-sub').onclick=()=>{
        ...
        if(hl.on==='是'){const no=p.querySelector('#st-hl-no');if(!no.value.trim()){no.classList.add('err');no.focus();toast('请填写 HALAL 证书编号');return;}if(!/^[A-Za-z0-9\-\/ ]{1,50}$/.test(no.value.trim())){no.classList.add('err');no.focus();toast('证书编号仅支持字母、数字、- / 和空格');return;}if(!hl.files.length){toast('请上传 HALAL 证书');return;}}
        STORE_ABBR=inp.value.trim();STORE_HALAL_REJECT='';
...
/* ============ 营业时间与截单（对齐 PC「店铺管理·营业管理」：每日营业时段 start–end 可配，
   截止时间即当日截单时间；截单后统一停止接单——客户须等下一个营业时间才可下单，无 T+2 顺延模式） ============ */
const PLATFORM_CUTOFF='18:00';  // 平台公共截单时间(平台级上限)；商家截止不得晚于此，入驻默认初始化 00:00~此
const BIZ_WEEK=[
  {d:'周一',on:true,start:'00:00',end:'15:00'},
  {d:'周二',on:true,start:'00:00',end:'15:00'},
  {d:'周三',on:true,start:'00:00',end:'15:00'},
  {d:'周四',on:true,start:'00:00',end:'15:00'},
  {d:'周五',on:true,start:'00:00',end:'15:00'},
...
function bizPeriod(w,nowHM){if(!w.on)return 'rest';if(nowHM<w.start)return 'before';if(nowHM<w.end)return 'open';return 'ended';}  // 今日三段
function bhCard(w,i){
  const isT=i===bizTodayIdx();const nowHM=bizNowHM();const per=isT?bizPeriod(w,nowHM):'';
  const lockTog=per==='open';const lockSt=per==='open';const endMin=per==='open'?nowHM:'';
  const badge=isT?` <span style="font-size:10px;color:var(--emerald-2);background:var(--mint-soft);padding:1px 6px;border-radius:6px;margin-left:6px">今日·${({rest:'休息',before:'未开始',open:'营业中',ended:'已结束'})[per]}</span>`:'';
  const todayHint={rest:'今日休息 · 可改营业状态',before:'今日尚未开始 · 营业状态/开始/截单均可改（即时生效）',open:`<span style="color:var(--red)">今日营业中 · 仅可改截单（${nowHM}~${PLATFORM_CUTOFF}）· 营业状态/开始时间锁定</span>`,ended:`<span style="color:var(--red)">今日营业已结束 · 改动将于下周${w.d}生效</span>`}[per];
  return `<div class="bh-card">
    <div class="bh-hd"><span class="bh-day">${w.d}${badge}</span><span class="bh-st">${w.on?'营业':'休息'}</span><span class="bh-sw ${w.on?'on':''}" ${lockTog?'style="opacity:.45"':`data-on="${i}"`}><span class="dot"></span></span></div>
    ${w.on?`
    <div class="bh-row"><span class="bh-lb">营业时段</span><span class="bh-time"><input type="time" ${lockSt?'disabled':`data-start="${i}"`} value="${w.start}"> – <input type="time" data-end="${i}" ${endMin?`min="${endMin}"`:''} max="${PLATFORM_CUTOFF}" value="${w.end}" style="${w.end>PLATFORM_CUTOFF?'border-color:var(--red);color:var(--red)':''}"></span></div>
    ...
    `:`<div class="bh-rest">${isT?'今日休息 · 可改营业状态（开业则可设时段）':'今日休息，前台展示「今日休息」，客户当天不可下单'}</div>`}
  </div>`;
}
function openBizHours(){
  pushPage({title:'营业时间设置',body:`
    <div class="bh-tip">平台公共截单时间 <b>${PLATFORM_CUTOFF}</b>，截止不得晚于它。<b>T+1 及以后</b>自由改；<b>今日三段</b>：未到开始→营业状态/开始/截单均可改（即时）；营业中→仅改截单（状态/开始锁定，不早于现在、不晚于平台截单）；营业结束→改动下周对应日生效。截单后停止接单，客户须等下一个营业时间才可下单。入驻默认 00:00 ~ ${PLATFORM_CUTOFF}、全周营业。</div>
    ...
    mount:(p)=>{
      ...
      b.onclick=()=>{
        ...
        for(let i=0;i<BIZ_WEEK.length;i++){const w=BIZ_WEEK[i];if(!w.on)continue;
          ...
          if(i===ti){const per=bizPeriod(w,nowHM);
            if(per==='open'&&w.end<nowHM)return toast(`今日营业中，截单时间不能早于当前时间 ${nowHM}`);
            if(per==='ended')weekEffect=true;}}
...
function openQual(){
  pushPage({title:'经营资质',body:`
    ...
    ${STORE_HALAL.on==='是'?`<div class="mn-qz">
      ...
      <div class="qr"><span class="qk">有效期</span><span class="qv">—</span></div>
      <div class="qr"><span class="qk">状态</span><span class="qv">有效</span></div>
    </div>`:''}
    <div class="mn-qz">
