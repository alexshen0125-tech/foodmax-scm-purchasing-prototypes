...
/* 逐日链式演算（与 PC pc-modules/presend.js 同算法同数字）
   ...
   应送 = max(0, max(订单量, 预送量) − 期初在仓)：预送量是全天预测总量、已含订单量，取大不相加，再扣期初在仓 */
  ...
  AUD_DAYS.slice().reverse().forEach(d=>{
    ...
    whs.forEach(wh=>{
      ...
      const lines=rs.map(r=>{
        ...
        const need=Math.max(orderQty,psQty);                                // 当日需求 = 取大（预送量含订单量）
        const ded=Math.min(open,need),psNet=need-ded;
        const planned=Math.max(0,need-open);                                 // 应送 = max(0, 需求 − 在仓剩余)
        const h=hnum(sd+'rc',10);
        const received=h<2?Math.max(0,planned-(1+hnum(sd+'sd',5)))
                      :h<4?planned+(2+hnum(sd+'od',6))
                      :planned;
        const quota=Math.max(orderQty,psQty), stock=open+received;          // 可卖上限 = 全天预测总量（含订单）
        const demand=Math.max(1,Math.round(need*(0.7+hnum(sd+'ra',45)/100)));
        ...
        carry[key]=close;
        return {sku:r.sku,name:r.name,unit:r.unit,orderQty,psQty,need,open,ded,psNet,planned,received,sold,out:outQty,close,
...
// BR-15b（2026-09-23 拍板）：需求 = max(订单量, 预送量)，应送 = max(0, 需求 − 在仓剩余)——预送量含订单量，取大不相加
...
/* ---------- 在仓预送库存 ---------- */
let STAB='all';
// 次日应送量（BR-15b）：max(次日订单需求, 次日预送量 − 在仓剩余)——预送量含订单量，不相加
function nextPsOf(s){const ps=ROWS.find(x=>x.sku===s.sku&&x.wh===s.wh);return (psOn()&&ps)?finalQty(ps):0;}
function nextShouldOf(s){return shouldOf(s.nextNeed,nextPsOf(s),s.left).should;}
...
function drawStock(box){
  ...
  box.innerHTML=`
    <div class="ps-note" style="margin-top:12px">当日预送到仓、截单后没卖完的货留在仓里，<b>次日的订单量与预送量一起先扣它</b>——次日应送量 = max(0, max(次日订单量, 次日预送量) − 在仓剩余)，扣完就是次日免送。你<b>多送</b>的部分仓库照收，也直接进这里（当天不参与售卖）。货权归你，<b>滞销与临期由你处理</b>、可申请退回；保管期间的<b>损坏与丢失由平台承担</b>。</div>
    <div class="ps-tabs">
...
function openStockDetail(s){
  ...
  pushPage({title:s.name,body:`
    ...
    <div class="ps-tbl">
      ...
      ${(()=>{const g=nextPsOf(s),sp=shouldOf(s.nextNeed,g,s.left);return `<div class="ps-row"><span class="k">次日需求<br><span style="font-size:11.5px">max(订单 ${s.nextNeed}, 预送 ${g})</span></span><span class="v">${sp.need} ${s.unit}</span></div>
      <div class="ps-row"><span class="k">减去在仓剩余</span><span class="v">− ${sp.ded} ${s.unit}</span></div>`;})()}
      <div class="ps-row"><span class="k">次日应送量</span><span class="v hl">${need} ${s.unit}</span></div>
    </div>
...
function drawAudit(box){
  ...
  box.innerHTML=`
    <div class="ps-note" style="margin-top:12px">盘每天<b>送了多少、仓库收了多少、卖了多少、还剩多少</b>。<b>按送货单</b>看某天某仓这一单的结果，<b>按 SKU</b> 看某个品逐日的明细。<br>一天的账：<b>期初在仓 → 应送（<b>max(0, max(订单量,预送量) − 期初在仓)</b>，预送量含订单量不相加）→ 实收 → 卖出 → 仓库实出 → 期末在仓</b>，账必平（期末在仓 = 期初 + 实收 − 仓库实出，实出少于卖出 = 仓库少发）；期末在仓就是明天的期初，<b>明天的应送先扣它</b>。<br>${psOn()?'应送 = max(0, max(订单量, 预送量) − 在仓剩余)，预送量是全天预测总量、已含订单量，取大不相加；<b>短收</b>按实收计、当日配额同步下调；<b>多收</b>仓库照收不设上限，已入在仓寄存，当天不参与售卖、次日优先抵扣。':'应送 = 订单量；<b>短收</b>按实收计；<b>多收</b>仓库照收不设上限，已入在仓寄存，次日优先抵扣。'}</div>
...
function openDocDetail(d){
  ...
  pushPage({title:d.no,body:`
    ...
    <div class="ps-tbl">
      ...
      <div class="ps-row"><span class="k">应送（取大后扣期初在仓）</span><span class="v">${t.planned}</span></div>
    ...
    <div class="ps-tbl">
      ${d.lines.map(l=>`<div class="ps-row"><span class="k" style="flex:1;text-align:left">
        <b style="color:var(--ink)">${l.name}</b> ${l.psQty?'<span class="ps-tag" style="background:var(--amber-soft);color:#8A5A12">预测送货</span>':'<span class="ps-tag" style="background:var(--mint-soft);color:var(--emerald-2)">实际送货</span>'}<br><span style="font-size:11.5px">订单 ${l.orderQty}${l.psQty?' · 预送 '+l.psQty:''} · 取大 ${l.need}${l.ded?' · 抵扣在仓 −'+l.ded:''}</span></span>
  ...
  pushPage({title:r.name,body:`
    ...
    <div class="ps-tbl">
      ${r.days.map(x=>`<div class="ps-row"><span class="k" style="flex:1;text-align:left">
        <b style="color:var(--ink)">${x.date}</b><br><span style="font-size:11.5px">${x.no} · 订单 ${x.orderQty}${x.psQty?' · 预送 '+x.psQty:''} · 取大 ${x.need}${x.ded?' · 抵扣在仓 −'+x.ded:''}</span></span>
    ...
    <div class="ps-tbl">
      ...
      ${(()=>{const sp=shouldOf(todayOrder,todayPs,left);return `<div class="ps-row"><span class="k">今日需求<br><span style="font-size:11.5px">max(订单 ${todayOrder}, 预送 ${todayPs})，预送量含订单量</span></span><span class="v">${sp.need} ${r.unit}</span></div>
      <div class="ps-row"><span class="k">减去在仓剩余（上表期末在仓）</span><span class="v">− ${sp.ded} ${r.unit}</span></div>`;})()}
      <div class="ps-row"><span class="k">今日应送</span><span class="v hl">${todayShould} ${r.unit}</span></div>
    </div>
    <div style="height:16px"></div>`});
}
