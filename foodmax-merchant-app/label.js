/* Food Max 商家端 v2 · 打印标签（自 PC「备货管理 › 打印标签」搬迁，字段对齐 PC）
   按「配送日期 + 仓库 + 商品名称」把待发货订单聚合到 SKU × 仓库，
...
const state={date:null,wh:'',name:''};   // 筛选：配送日期 / 仓库 / 商品名称
...
function renderBody(box){
  ...
  box.innerHTML=`
    <div class="lb-note">🏷️ 按「配送日期 + 仓库」汇总各 SKU 应送货张数（每件一张，序号连续）。多退少补商品按 SKU 打标、印实发净重，不含订单/客户信息，货到仓由 WMS 统一分拣。</div>
    <div class="lb-filter">
