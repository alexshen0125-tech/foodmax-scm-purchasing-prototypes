/* Food Max 商家端 v2 · 自营补货（与 PC「财务 › 自营补货」pc-modules/replenish.js 同口径）
   适用范围：仅【出库前·仓内收货清点】场景；差异只有一种——实收 < 应送（数量少了），无原因分类。
   业务链：送货到仓 → 仓库收货清点少货 → 自营现货可【全额覆盖】缺口 → 生成补货单
           → 平台按【含税售价 ×(1+加价率)】向商家销售缺口数量 → 当期结算单抵扣 → 平台开补货销售发票。
   口径（2026-08-07 缺货补货方案对焦会 + 沈亮 2026-08-10 确认）：
   - 客户订单完全无感（商品/金额/发票不变）；商家 GMV 与佣金按【应送数量】足额计，不因少货下调。
   - 补货单价(含税) = 含税售价 ×(1+加价率)；加价率【全平台统一】单值，默认 30%、平台可配、生成时快照，
     不分商家/品类/site；售价取原订单下单时快照。
   - 等效罚则 = 缺口数量 × 含税售价 × 加价率。
   - 自营现货不足以全额覆盖缺口 → 不生成补货单，按实收数量出库并标缺货。
...
function openReplen(){
  ...
  pushPage({title:'自营补货',body:`
    ...
    <div class="rp-search"><input id="rp-q" placeholder="搜补货单号 / 送货单号 / 原订单号" value="${_q}"><span class="clr" id="rp-clr">✕</span></div>
...
function openDetail(r){
  if(!r)return;
  pushPage({title:'补货单详情',navbar:true,body:`
    <div class="rp-card2">
    ...
    <div class="rp-card2">
      ...
      <div class="rp-row"><span class="k">补货单价 = ${money(r.price)} ×(1+${r.rate}%)</span><span class="v">${money(unit(r))}</span></div>
      ...
      <div class="rp-row total"><span class="k">补货款（含税 · 结算抵扣）</span><span class="v neg">-${money(amt(r))}</span></div>
    </div>
    ...
    <div class="rp-card2">
      ...
      <div class="rp-note">平台就补货单向你开具<b>补货销售发票</b>（开票方＝平台、收票方＝商家，GST ${GST}%），结算完成后自动开具并推送，你只需查看/下载。<br>对<b>实收数量</b>有异议，请在<b>收货清点后 7 个自然日内</b>联系你的运营对接人（微信群「Food Max 供应商-绿鲜源」/ +65 6123 4567），并提供<b>装车照片或司机交接单</b>；平台调取仓库收货监控核对，核实有误的由运营发起<b>冲正</b>，下期结算回补并开红冲发票。逾期以清点数量为准。</div>
    </div>
