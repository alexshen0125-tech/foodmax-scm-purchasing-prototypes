...
const PRODUCTS=[
  ...
  {ic:'🥥',n:'鲜丰 · NFC 椰子水 330ml',code:'SPU8820',cat:'饮料',halal:1,bcrs:true,bcrsUnitContainers:1,sales:11,createdAt:'2026-07-10 10:20',updatedAt:'2026-07-21 18:00',
  ...
  {ic:'🍗',n:'鲜丰 · 鸡胸肉',code:'SPU8818',cat:'肉禽蛋品',halal:1,status:'reviewing',sales:0,createdAt:'2026-08-01 09:10',updatedAt:'2026-08-01 09:10',
...
function openGoodsDetail(g,s){
  ...
  const info=[['商品名称',g.n],['商品名称(EN)',g.nEn||'—'],['商品别名',g.alias||'—'],['商品别名(EN)',g.aliasEn||'—'],
    ...
    ['BCRS 押金',g.bcrs?`<span class="bcrs-b" style="margin:0 5px 0 0">BCRS</span>每最小售卖单位 ${(+g.bcrsUnitContainers||0)} 容器 × S$${BCRS_UNIT_PRICE.toFixed(2)}（单容器法规押金 S$${BCRS_UNIT_PRICE.toFixed(2)}，平台固定·不计 GST）`:'不支持'],
    ...
    <div class="card">
      ...
      ${g.skus.map(x=>{const oos=(+x.stock<=0);const stt=aud?'随商品':(x.off?'已下架':(oos?'售罄':'在售'));const cur=x===s;const xb=skuBcrs(g,x);
        return `<div class="sku${cur?' cur':''}">
          ...
          ${xb?`<div class="ln">BCRS 押金 <b>S$${xb.toFixed(2)}</b>（${skuContainers(g,x)}容器 × S$${BCRS_UNIT_PRICE.toFixed(2)}，不计 GST）</div>`:''}
...
function renderList(container,inTab){
  container.innerHTML=`
    ...
    ${(window.FM_GOODS_FILTER||{}).halal===1?`<div class="gd-sub" style="padding-bottom:0"><span id="hf-chip" style="background:var(--gl,#E7F2E9);color:var(--g,#0E7A52);border-radius:14px;padding:3px 10px;font-size:12px;font-weight:600">仅看 HALAL 商品 ✕</span></div>`:''}
  ...
  const HF=(window.FM_GOODS_FILTER||{}).halal===1;   // 店铺 HALAL 是→否 拦截跳转带入的过滤（不露出筛选控件）
  const hfc=container.querySelector('#hf-chip');if(hfc)hfc.onclick=()=>{window.FM_GOODS_FILTER=null;renderList(container,inTab);};
  const flatSkus=(tab)=>{const arr=[];PRODUCTS.forEach((g,gi)=>(g.skus||[]).forEach((s,si)=>{
    if(HF&&g.halal!==1)return;
...
function openPrice(g,only){
  ...
  pushPage({title:'改价格',body:`<div style="height:4px"></div>
    ...
    <div class="sku-ed-tip">${g.bcrs?`<b>BCRS 商品</b>：容器数在商品信息里维护、押金单价平台固定 S$${BCRS_UNIT_PRICE.toFixed(2)}/容器，此处不改；押金为<b>过手项</b>，不计 GST、不计入佣金基数与预计收入。<br>`:''}价格维护到每个售卖规格(SKU)，录入<b>未税价</b>，系统按税率 ${rate}% 自动算含税价；提交即时生效。<b>商品佣金 = 含税价×服务费率(${(SVC*100).toFixed(0)}%)</b>，<b>上门揽收费</b>为单独固定字段(与价格/比例无关)，<b>预计收入 = 含税价−佣金−揽收费</b>；仅供参考，以订单结算为准。</div>
